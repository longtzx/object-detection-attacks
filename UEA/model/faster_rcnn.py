...
class FasterRCNN(nn.Module):
    ...
    def forward(self, x, scale=1., flag=False):
        ...
        * :math:`L` is the number of classes excluding the background.
        ...
        h = self.extractor(x).data

        if flag:
            rois, roi_scores = \
                self.rpn(h, img_size, scale, flag=flag)
            del h
    ...
    def _suppress(self, raw_cls_bbox, raw_prob, gt_labels, attack=False):
        ...
        for l in range(1, self.n_class):
            ...
            # keep = nms(cp.array(cls_bbox_l), self.nms_thresh, prob_l)
    ...
    def predict(self, imgs, sizes=None, visualize=False, attack=False, \
        ...
        for img, size in zip(prepared_imgs, sizes):
            ...
            if not attack:
                ...
                # bbox, label, score = self._suppress(raw_cls_bbox, raw_prob, None)
                bbox, label, score = self._suppress(cls_bbox, prob,None)
            else:
                prob = F.softmax(at.totensor(roi_score),dim=1)
                raw_cls_bbox = at.tonumpy(cls_bbox)
                raw_prob = at.tonumpy(prob)
                # bbox, label, probs, mask = self._suppress(raw_cls_bbox,prob, None, attack=True)
                bbox, label, probs, mask = self._suppress(cls_bbox, prob, None, attack=True)
