from rap.attack_wrapper import AttackWrapper
import numpy as np
from rap.rap_utils import select_detections
from rap import attacks
import torch
...
class FRAttackWrapper(AttackWrapper):
    def search_candidates(self, target_bbox, target_bbox_label, rpn_cls_prob, rpn_bboxes):
        ...
        """
        #0.01 改为 0.4,0.3 改为 0.1
        # Select predictions which have large overlap with targets set
        rpn_labels_break = (rpn_cls_prob[:, 1] > 0.4).astype(np.int32)
        ...
        # print(rpn_labels_break.shape)
        # print(target_bbox_label.shape)
