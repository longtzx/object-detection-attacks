...
sys.path.append(os.path.abspath('/home/yl/PycharmProjects/TOG-master/frcnn_utils/pytorch-faster-rcnn/lib'))
sys.path.append(os.path.abspath('/home/yl/PycharmProjects/TOG-master/frcnn_utils/pytorch-faster-rcnn/tools'))
from model.bbox_transform import clip_boxes, bbox_transform_inv
from nets.vgg16 import vgg16
from model.config import cfg
import copy
from dataset_utils.preprocessing import get_new_img_size
...
class FRCNN_RAP(nn.Module):
    ...
    def predict_all(self, blobs):
        ...
        scores = np.reshape(scores, [scores.shape[0], -1])  # 添加-1是因为不知道具体的值
...
class FRCNN(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        sample_roi, gt_roi_loc, gt_roi_label = self.proposal_target_creator(roi, at.tonumpy(bbox), at.tonumpy(label), self.loc_normalize_mean,
    ...
    def detect_test(self, image, iou_threshold=0.30, conf_threshold=0.70):

        _bboxes, _labels, _scores, _logits = self.faster_rcnn.predict(image, conf_threshold, iou_threshold)
        _bboxes = _bboxes[0][:, [1, 0, 3, 2]]  # ymin, xmin, ymax, xmax --> xmin, ymin, xmax, ymax
        _labels = np.expand_dims(_labels[0], axis=1)
        _scores = np.expand_dims(_scores[0], axis=1)
        _logits = _logits[0]
